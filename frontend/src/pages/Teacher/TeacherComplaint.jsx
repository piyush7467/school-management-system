...
const TeacherComplaint = () => {
  ...
  // Fetch all complaints by this teacher
  const fetchComplaints = async () => {
    try {
      setLoading(true);
      const res = await API.get("/teacher/auth/complaint/my");
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await API.post("/teacher/auth/complaint", { title, description });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-blue-100 dark:from-slate-900 dark:via-indigo-900/20 dark:to-blue-900/20 py-6 px-4 lg:px-8">
            ...
            <Card className="border-0 shadow-xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm sticky top-6">
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white shadow-lg transition-all duration-200"
                >
                  {submitting ? (
                    <>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-12 space-y-3">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-lg">
                      {activeTab === "all"
                        ? "No complaints yet."
                        : `No ${activeTab} complaints.`}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-500">
                      {activeTab === "all"
                        ? "Submit your first complaint to get started."
                        : "All complaints are filtered out."}
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                    {filteredComplaints.map((complaint) => {
                      const StatusIcon =
                        statusConfig[complaint.status]?.icon || Clock;
                      return (
                        <div
                          key={complaint._id}
