...
const TeacherMarkAttendance = () => {
  ...
  const loadStudents = () => {
    ...
    const sectionData = assignedSections.find(
      (s) =>
        String(s.classId) === selectedClass &&
        String(s.sectionId) === selectedSection
    );

    if (!sectionData) return setStudents([]);

    const normalized = (sectionData.students || []).map((st) => ({
      _id: st._id || st.studentId,
      firstName: st.firstName,
      lastName: st.lastName,
      username: st.username,
      status: "Present",
      avatar:  st.profilePic || userLogo || `https://api.dicebear.com/7.x/avataaars/svg?seed=${st.username || st._id}`
    }));
  ...
  useEffect(() => {
    loadStudents();
  }, [selectedClass, selectedSection, assignedSections]);

  // 🔹 Toggle status (Present → Absent → Late)
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      await API.post(
        `/teacher/auth/attendance/class/${selectedClass}/section/${selectedSection}/mark`,
        { 
          students: formatted,
          date: date
        }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                Mark attendance for your assigned class sections
              </p>
            </div>
          </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    {[...new Map(
                      assignedSections.map((a) => [a.classId, a.className])
                    )].map(([id, name]) => (
